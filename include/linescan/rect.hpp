...
namespace linescan{
	...
	public:
		...
		rect(linescan::size< size_type > const& size):
			size_(size)
			{}
		...
		rect(point< position_type > const& top_left, linescan::size< size_type > const& size):
			top_left_(top_left),
			size_(size)
			{}
		...
		/// \brief Get the size
		linescan::size< size_type > const size()const{
			return size_;
		}
		...
		void set_size(linescan::size< size_type > const& size){
			size_ = size;
		}
